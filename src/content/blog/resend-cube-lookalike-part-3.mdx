...
image: /images/resend-part-3-og.png
...
This is the final installment of our Resend Cube Lookalike series, where we focus on perfecting the recreation of the Resend Cube. This time we fix the lights and introduce "god rays," an impressive lighting technique to enhance the cube's visual appeal. You can get caught up with part 1, <a class="internal-link" href="/blog/making-the-resend-cube-from-scratch-using-three-js">where we have built the cube itself</a>, and part 2, <a class="internal-link" href="/blog/resend-cube-lookalike-part-2">where we have added animations and some cool effects</a>.

## Lights

The current lights setup is shown below:

<Image src="/blog/resend-cube-lookalike-part-3/old-lights.png" width="640" alt="The Current Lights Setup" />

The effect it creates is not ideal - we never have a whole side of the cube highlighted, but always only a circle within the side - demonstrated evident in the video below. To make the lightning look better, we can use different kind of lights - `RectAreaLight`.

<Image src="/blog/resend-cube-lookalike-part-3/point-lights.gif" width="720" alt="Visualization of the Point Lights" />

RectAreaLights are literally just panes which emit light. If we make them big enough, we can make the whole side of the cube highlighted. To make the RectAreaLights work though, we have to change the controls of our cube - instead of orbit controls, we need to rotate the whole cube. This change enhances the lighting effect on the cube.

Starting with some imports, we add the following lines to our importmap (if you don't have an importmap to add to, you might want to check the <a class="internal-link" href="/blog/resend-cube-lookalike-part-2">part 2</a> of this series):
...
Rotating the cube, you can see it looks much better. You can also see a huge pane of light, which is unwanted, requiring removal. This is where rotating the cube, rather than using orbit controls, becomes beneficial.

### Rotation

Using code inspired by this <a class="external-link" href="https://jsfiddle.net/MadLittleMods/n6u6asza/" target="_blank">jsfiddle</a>, we can rotate the cube on drag instead of rotating the camera, which stays fixed - allowing us to place the lights outside of the camera view, but still casting light on the cube.
...
<Image src="/blog/resend-cube-lookalike-part-3/new-controls.png" width="828" alt="The New Rotation Setup" />
...
Our next step is to remove all logic regarding the orbit controls and replace it with the "drag controls". Here are the lines to remove in the code:
...
If done correctly, the cube's rotation ability will be temporarily disabled and the page still runs fine inside your browser. Now let's steal some code from the jsfiddle. We will add a "Controls" section right before animating the cube and a call of the update function right after calling animate():
...
You should be able to rotate the cube again. Awesome. Now for the fancy lights: you'll notice the cube appears faded, losing direct lighting on its sides. We need to bring that back by adding more rect lights.
...
<Image src="/blog/resend-cube-lookalike-part-3/new-light-setup.png" width="828" alt="The New Lights Setup" />

Pretty sick. I encourage you to play with the light and find out what you like. Tip: To adjust light positioning, pause the animation by commenting out these lines:
...
Now, let's explore an effective technique I believe the Resend team used (might be a complete conspiration by me) the Resend guys did - using the god rays to fix a flaw of the cube where it sometimes almost entirely blends into the background. The image below demonstrates the issue.

<Image src="/blog/resend-cube-lookalike-part-3/non-visible-cube.png" width="828" alt="The Almost Non-visible Cube" />

To fix this (or at least I think that's what happened), they introduced an element serving both as a "light source", but more importantly serving as a background for the cube in the case it gets too dark. A lighter background ensures the cube's outline remains visible, preventing you from losing it (suddenly realise you don't know what you are looking at).
...
This might seem like and easy task - remove the scene background and we are done. At least that's what I thought. However, it's not as straightforward as it seems.

Let's start by removing the scene background with this line: `scene.background = new THREE.Color( 0x000000 );`. We should then set the rendering to support opacity by adding alpha to renderer options:
...
We still can't see the light ray. That is, as I've found out (<a class="external-link" href="https://discourse.threejs.org/t/transparent-background/22742" target="_blank">here</a> and <a class="external-link" href="https://stackoverflow.com/questions/20899326/how-do-i-stop-effectcomposer-from-destroying-my-transparent-background" target="_blank">here</a>) because our Bloom pass does not support opacity. The solution - replace the rendering pipeline.

We will replace our render pass and our effects pass from three with effects from <a class="external-link" href="https://github.com/pmndrs/postprocessing" target="_blank">postprocessing</a> library by <a class="external-link" href="https://github.com/pmndrs" target="_blank">Poimandres</a>. Let's start by importing it (add to importmap + call inside script)
...
<Image src="/blog/resend-cube-lookalike-part-3/god-rays.png" width="720" alt="The god Rays" />
...
Thanks for reading this post (and the series). It's been fun and insightful for me to make it. I certainly have learned a ton of stuff and hope you have as well. I'd be happy to hear any feedback either via email at <a href="mailto:hi@devslovecoffee.com">hi@devslovecoffee.com</a>, or via <a class="external-link" href="https://twitter.com/devslovecoffee" target="_blank">Twitter</a>.
...
<a class="internal-link" href="/blog/making-the-resend-cube-from-scratch-using-three-js">Part 1 with the basics is here</a>.
<a class="internal-link" href="/blog/resend-cube-lookalike-part-2">Part 2 with the Bloom and Animation here</a>.